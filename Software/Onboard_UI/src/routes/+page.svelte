<script lang="ts">
	...
	} from '$lib/services/sensor-diagnostics';
	...
	onMount(() => {
		const clearActions = setInputContext('home', {
			onSingle: () => {
				handleSinglePress();
				void refreshSensors();
			}
		...
		<section class="rondell-wrap" aria-label="Sensordaten Rondell">
			<div class="rondell">
				{#each sensorTiles as tile, index}
					<article
						class="sensor-tile {index === activeIndex ? 'is-active' : ''}"
						style={`--index:${index}; --total:${sensorTiles.length};`}
					>
			...
			<p class="connection-state">
				Status: {diagnostics.connection === 'connected' ? 'Sensoren online' : 'Fallback / offline'} ·
				Update: {diagnostics.lastUpdated}
			</p>
		</section>
	{/if}
