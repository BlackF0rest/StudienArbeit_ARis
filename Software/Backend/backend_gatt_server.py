...
class TextService(Service):
    ...
    def __init__(self, bus, index):
        super().__init__(bus, index, self.TEXT_SERVICE_UUID, True)
        self.add_characteristic(TextRxCharacteristic(bus, 0, self))
        self.add_characteristic(TextTxCharacteristic(bus, 1, self))
...
class TextService(Service):
    ...
    def __init__(self, bus, index):
        super().__init__(bus, index, self.TEXT_SERVICE_UUID, True)
        self.tx = TextTxCharacteristic(bus, 0, self)
        self.rx = TextRxCharacteristic(bus, 1, self)
        self.add_characteristic(self.rx)
        self.add_characteristic(self.tx)


class TextRxCharacteristic(Characteristic):
    """
    RX: Laptop -> Pi (Write)
    """
...
class TextTxCharacteristic(Characteristic):
    ...
    def StopNotify(self):
        self.notifying = False
        print("Client unsubscribed from TX notifications")
